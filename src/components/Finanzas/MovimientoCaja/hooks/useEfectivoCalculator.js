...
/**
 * Hook optimizado para cálculos de efectivo
 * Maneja la lógica de billetes, monedas y validaciones
 */
export const useEfectivoCalculator = (formData, handleInputChange) => {
    ...
    // Validación de efectivo - MEMOIZADA
    const validacionEfectivo = useMemo(() => {
        ...
        if (tieneErrorDesglose) {
            return {
                esValido: false,
                mensaje: `El desglose de efectivo (S/ ${totalCalculado.toFixed(2)}) no coincide con el monto (S/ ${parseFloat(formData.monto || 0).toFixed(2)})`
            };
        }
        ...
        if (totalCalculado === 0 && parseFloat(formData.monto || 0) > 0) {
            return {
                esValido: false,
                mensaje: 'Debe especificar el desglose de efectivo'
            };
        }
